import type * as EventTypes from '../event-types/asset_bridge';
...
export default class EventsClass {
	...
	public subscribeOnDepositRevertedEvent(callback : (event : EventTypes.DepositReverted) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('DepositReverted', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.DepositReverted);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'DepositReverted');
	}

	public subscribeOnExecuteWithMessageEvent(callback : (event : EventTypes.ExecuteWithMessage) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('ExecuteWithMessage', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.ExecuteWithMessage);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'ExecuteWithMessage');
	}

	public subscribeOnExecuteEvent(callback : (event : EventTypes.Execute) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Execute', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.Execute);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'Execute');
	}

	public subscribeOnTokenTransferEvent(callback : (event : EventTypes.TokenTransfer) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('TokenTransfer', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.TokenTransfer);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'TokenTransfer');
	}

	public subscribeOnTokenTransferWithInstructionEvent(callback : (event : EventTypes.TokenTransferWithInstruction) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('TokenTransferWithInstruction', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.TokenTransferWithInstruction);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'TokenTransferWithInstruction');
	}
