...
import { ContractFile } from '../contract-info/asset_bridge';
...
export default class Constructors {
	...
	* @param { (string | number | BN) } startNonce,
	*/
   	async "new" (
		routerBridge: string,
		chainId: string,
		...
		const storageDepositLimit = __options?.storageDepositLimit;
			const tx = code.tx["new"]!({ gasLimit, storageDepositLimit, value: __options?.value }, routerBridge, chainId, gateway, routerChainId, startNonce);
			let response;
